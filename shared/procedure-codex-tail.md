...
Codex auto-découvre les skills depuis `~/.agents/skills/`, `.agents/skills/` (repo), `/etc/codex/skills`. Vérifie au boot que `genius` est listé. Si absent : lis `~/.codex/skills/genius/SKILL.md` ou demande à la main de l'installer. Ne procède pas sur une tâche non-L1 sans avoir la table L1-L5 + le format bayésien en contexte.
...
**Discipline genius pour le sous-agent** : inclus dans son prompt : *"Avant d'agir, lis et applique `~/.agents/skills/genius/SKILL.md` (ou `~/.codex/skills/genius/SKILL.md` selon ton installation)."*
